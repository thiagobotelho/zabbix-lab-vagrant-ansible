Vagrant.configure("2") do |config|
  nodes = [
    { name: "zabbix_db",       ip: "192.168.56.30", ram: 2048, cpus: 2, playbooks: ["zabbix_db_setup.yaml", "zabbix_agent_install.yaml"] },
    { name: "zabbix_server",   ip: "192.168.56.60", ram: 1024, cpus: 1, playbooks: ["zabbix_server_install.yaml"] },
    { name: "zabbix_proxy",    ip: "192.168.56.50", ram: 1024, cpus: 1, playbooks: ["zabbix_proxy_install.yaml", "zabbix_agent_install.yaml"] },
  ...
  nodes.each do |node|
    config.vm.define node[:name] do |n|
      ...
      if node[:playbooks]
        node[:playbooks].each do |playbook|
